...
<script type="text/javascript">
    ...
    function deleteBatch(ispage) {
        ...
        //http://localhost:8080/admin/product.jsp
        var url = "<%=basePath%>/admin/product.jsp";
            //取得所有被选中删除商品的pid
            ...
            if(zhi.length==0){
                ...
                //拼接ID
                    ...
                    //发送请求到服务器端
                    <%--window.location="${pageContext.request.contextPath}/prod/deleteBatch.action?ids="+str+"&page=" + ispage;--%>
                    $.ajax({
                        url:"${pageContext.request.contextPath}/prod/deleteBatch.action",
                        data:{"pname":pname,"typeid":typeid,"lprice":lprice,"hprice":hprice,"page":ispage,"ids":str},
                        type:"post",
        ...
        if (confirm("确定删除吗")) {
          //向服务器提交请求完成删除
            $.ajax({
                ...
                data:{"pid":pid,"pname":pname,"typeid":typeid,"lprice":lprice,"hprice":hprice,"page":ispage},
                type:"post",
                success:function (msg) {
                    alert(msg);
                    //重新加载分页显示的组件table
            ...
            <%--window.location="${pageContext.request.contextPath}/prod/delete.action?pid="+pid+ "&page=" + ispage;--%>
        }
    }
...
<script type="text/javascript">
    $(function (){
        if ("${vo}" != "") {
            $("#pname").val(${vo.pname});
            $("#typeid").val(${vo.typeid});
            $("#lprice").val(${vo.lprice});
            $("#hprice").val(${vo.hprice});
        }
    })
